...
class DataProcessor:
    ...
    def _process_file(self, json_file):
        ...
        drug_name = json_file.get("drug_name_ru")
        if not drug_name:
            return
        ...
        count_side_e = 0
        for category, effects in json_file.get("side_e_parts", {}).items():
...
if __name__ == "__main__":
    ...
    filename = "make_side_effect_dataset\\data\\side_e_dataset.json"
    with open(filename, "r", encoding="utf-8") as file:
        json_dataset = json.load(file)

    # for item in 

        processor._process_file()
