...
class BayesianTrainer:
    ...
    def _compute_loss(self, max_combinations: Optional[int] = None, random_combinations: bool = True) -> torch.Tensor:
        ...
        # Батчевый проход: формируем тензоры evidence для пакетов комбинаций
        losses = []
        batch_size = 64  # фиксированный размер батча, можно менять при необходимости

        for start in range(0, len(selected_combinations), batch_size):
            batch_combos = selected_combinations[start:start + batch_size]
            B = len(batch_combos)
            # Инициализируем маркером -1.0 — означает "значение не задано / не наблюдается".
            ...
            # Запускаем батчевой forward
            marginals_batch = self.model(evidence_tensor)  # shape (B, n_nodes)

            # Для каждой комбинации в батче вычисляем её loss (используя предсказания из батча)
            for bi, combo in enumerate(batch_combos):
                # print("combo:", combo)
                target_dict = {}
                for drug_idx in combo:
                ...
                loss = torch.sum((preds - targets/3) ** 2)
                if loss.item() != 0.0:
                    losses.append(loss)
