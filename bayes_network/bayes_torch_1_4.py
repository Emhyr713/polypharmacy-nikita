...
class BayesianTorchModel(nn.Module):
    def __init__(self, graph_data: dict, prob_data: Optional[dict] = None, device: Optional[torch.device] = None, logfile_path = None):
        ...
        for link in graph_data["links"]:
            ...
            # elif self.id_to_idx.get(link["target"], None):
            #     print("source:", link["source"], self.node_meta[link["source"]])
            # elif self.id_to_idx.get(link["source"], None):
            #     print("target:", link["target"], self.node_meta[link["target"]])
        ...
        # for id, meta in self.node_meta.items():
        #     if meta["label"] == "prepare":
        #         print(meta["name"], meta["label"])
        ...
        #                     for b in range(self.cpt_parents_count[i])])), 0.5)) if n_conf>1 
        #                     else float(cpt.get("",0.5)) for conf_int in range(n_conf)]
        #         probs_t = torch.tensor(probs, dtype=torch.float32, device=self.device)
        #         logits = torch.log(probs_t.clamp(1e-6,1-1e-6)/(1-probs_t.clamp(1e-6,1-1e-6)))
        #     elif node_label == "prepare" and n_conf == 1:
        #         print("NEN", node_label, n_conf, i)
        #         # узлы без родителей с label == "prepare" инициализируем с 0
        #         probs_t = torch.tensor([0.0], dtype=torch.float32, device=self.device)
        #         logits = torch.log(probs_t.clamp(1e-6,1-1e-6)/(1-probs_t.clamp(1e-6,1-1e-6)))
        #     else:
        #         # обычная случайная инициализация
        #         init_p = torch.rand(n_conf, device=self.device)*0.8 + 0.1
        #         logits = torch.log(init_p/(1-init_p))
        #     self.logit_params.append(nn.Parameter(logits))
    ...
    def forward(self, evidence: Optional[Dict[int, float]] = None) -> torch.Tensor:
        ...
        # Старый режим: evidence dict или пустой
...
class BayesianTrainer:
    ...
    # # --- исправление _process_combo: не включать нули в словарь evidence ---
    # def _process_combo(self, combo: List[int]) -> torch.Tensor:
    #     # Передавать в model только действительно наблюдаемые значения (1.0).
    #     evidence = {idx: 1.0 for idx in combo}
    ...
    def _compute_loss(self, max_combinations: Optional[int] = None, random_combinations: bool = True, max_drugs_per_combo: Optional[int] = None, epoch: Optional[int] = None, total_epochs: Optional[int] = None) -> torch.Tensor:
        if not self.orlov:
            return torch.tensor(0.0, device=self.device)

        n_drugs = len(self.drug_indices)
        all_combinations = []
        for i in range(1, 2**n_drugs):
            combination = []
            for j in range(n_drugs):
                if i & (1 << j):
                    combination.append(self.drug_indices[j])
            if max_drugs_per_combo is not None and len(combination) > max_drugs_per_combo:
                continue
        ...
        for start in range(0, len(selected_combinations), batch_size):
            ...
            for bi, combo in enumerate(batch_combos):
                ...
                for se_idx in target_dict:
                    ...
                    combined_prob = 1.0 - torch.prod(1.0 - probs)
                    target_dict[se_idx] = combined_prob.clamp(0.0, 1.0).item()
    ...
    def train_model(
        ...
        if curriculum_learning:
            log_to_file(self.logfile_path, f"Curriculum learning enabled. Max complexity: {max_complexity}")
        ...
        for epoch in range(1, epochs + 1):
            ...
            if curriculum_learning:
                ...
                current_max_drugs = max(1, min(max_complexity, int(1 + progress * (max_complexity - 1))))
            
            # current_max_drugs = None
            # if curriculum_learning:
            #     progress = (epoch - 1) / max(1, epochs - 1)
            #     # Используем более агрессивную функцию роста (квадратичная)
            #     complexity_progress = progress ** 0.5  # Более быстрый рост в начале
            #     # Вычисляем непрерывное значение и округляем до ближайшего целого
            #     raw_complexity = 1 + complexity_progress * (max_complexity - 1)
            #     current_max_drugs = max(1, min(max_complexity, round(raw_complexity)))
            #     # Гарантируем достижение максимальной сложности за 80% эпох
            #     if epoch > epochs * 0.8:
            #         current_max_drugs = max_complexity

            loss = self._compute_loss(
                max_combinations=max_combinations,
                random_combinations=random_combinations,
                max_drugs_per_combo=current_max_drugs,
                epoch=epoch,
                total_epochs=epochs
            )
            ...
            if epoch % log_interval == 0 or epoch == 1 or epoch == epochs:
                ...
                log_msg = f"Epoch {epoch}/{epochs} Loss: {total_loss.item():.8f} Raw Loss: {loss.item():.8f} LR: {current_lr:.6f}"
                if curriculum_learning:
...
if __name__ == "__main__":
    ...
    prob_data = f"{DIR}data/probabilities_opt_10_6_pytorch_test.json"
    ...
    final_probs, final_loss = trainer.train_model(
        ...
        final_file="data/probabilities_opt_pytorch_10_6.json",
