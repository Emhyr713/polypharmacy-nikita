...
class BayesianTrainer:
    ...
    def _compute_loss(self) -> torch.Tensor:
        ...
        losses = []
        for drug_idx in self.drug_indices:
            # evidence = {drug_idx: 1.0}
            evidence = {idx: 1.0 if idx == drug_idx else 0.0 for idx in self.drug_indices}
            marginals = self.model(evidence)
            se_entries = self.orlov[drug_idx]
            se_indices = torch.tensor([se for se, _ in se_entries], device=self.device)
            targets = torch.tensor([p for _, p in se_entries], device=self.device)
            preds = marginals[se_indices]
            losses.append(torch.sum((preds - targets) ** 2))
...
if __name__ == "__main__":
    ...
    graph_path = "data/graphs/graphs_10_5.json"
    orlov_path = "data/Orlov.json"
    logfile = "data/opt_calc_10_5.txt"
    trainer = BayesianTrainer(graph_path, orlov_path, logfile_path=logfile, device='cpu')
