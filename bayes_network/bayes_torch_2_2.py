...
class BayesianTrainer:
    ...
    def get_balanced_with_minimum(self, drug_indices, min_r, max_r, min_per_r=2000, max_per_r=2000):
        """
        Гарантирует хотя бы min_per_r комбинаций для каждого r.
        Если их меньше - повторяет существующие.
        """
        all_combinations = []
        
        for r in range(min_r, max_r + 1):
            # Генерируем все комбинации
            combos = list(combinations(drug_indices, r))
            
            # Если комбинаций меньше минимума
            if len(combos) < min_per_r:
                # Дублируем до достижения минимума
                repeats = (min_per_r + len(combos) - 1) // len(combos)  # ceiling division
                combos = (combos * repeats)[:min_per_r]
            # Если комбинаций больше максимума
            elif len(combos) > max_per_r:
                combos = random.sample(combos, max_per_r)
            
            all_combinations.extend([list(c) for c in combos])
    ...
    def _compute_loss(
        ...
        # Балансированная генерация выборки
        selected_combinations = self.get_balanced_with_minimum(self.drug_indices, min_r, max_r, min_per_r=4000, max_per_r=8000)
    ...
    def train_model(
        ...
        if optimizer_name.lower() == "adamw":
            ...
            optimizer = RAdam(params_to_optimize, lr=lr, weight_decay=weight_decay/3)
        else:
        ...
        LOGIT_MAX = prob_to_logit(0.5)    # = 0.0
...
if __name__ == "__main__":
    ...
    final_probs, final_loss = trainer.train_model(
        ...
        final_file=f"{DIR}data/probabilities_opt_{GRAPH_NAME}_pytorch_2_2_comb_3_v6.json",
        optimizer_name="AdamW",
        # optimizer_name="Adam",
        scheduler_name="cosineannealing",
        weight_decay=1e-5,
        grad_clip=1,
        max_combinations=12000,
        max_drug_in_comb=2,
        target_scale = 1
    )
