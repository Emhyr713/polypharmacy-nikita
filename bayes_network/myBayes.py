...
class BayesianNode:
    ...
    """
    def __init__(self, node_id, name, parents, child, label, prob_data,   parent_configs, cond_probs):
        self.id = node_id
        self.name = name
...
class BayesianGraph:
    def __init__(self, graph_data: dict, prob_data: Optional[dict] = None, logfile_path = 'opt_calc.txt'):
    ...
    def _build_network(self) -> Dict[str, BayesianNode]:
        ...
        for node_config in self.graph_data['nodes']:
            ...
            # node_prob_data = self.prob_data.get(node_name, {})
            ...
            nodes[node_id] = BayesianNode(
                ...
                parent_configs = np.array(keys, dtype=np.int8),  # или dtype=int
                cond_probs = np.array(values, dtype=np.float64)
            )
        log_to_file(self.logfile_path, f"Сборка словаря объектов BayesianNode завершена. Всего узлов: {len(nodes)}")
        return nodes
    
    def update_prob_data_in_network(self, prob_data):
        # print("update_prob_data_in_network: prob_data:", prob_data)
        for n_id, node in self.network.items():
            # new_prob_data_for_node = prob_data.get(node.name)
            new_prob_data_for_node = prob_data.get(node.id)
            node.update_prob_data_in_node(new_prob_data_for_node)

        # log_to_file(self.logfile_path, f"prob_data была изменена.")
        # log_to_file(self.logfile_path, f"Старая prob_data:{self.prob_data}")
        # log_to_file(self.logfile_path, f"Новая prob_data:{prob_data}")
        self.prob_data = prob_data
    ...
    # def sort_ids_by_reference(sorted_ids, ids_to_sort):
    #     """
    #     Сортирорует подможноство id согласно отсортированному списку.
    #     Может применяться для упорядочивания id родителей
    #     """
    #     index_map = {id_: i for i, id_ in enumerate(sorted_ids)}
    #     return sorted(ids_to_sort, key=lambda x: index_map[x])
    ...
    #                 ','.join(map(str, c)): round(random.uniform(0.01, 0.99), 6) for c in combs
    #             }
...
class BayesianNetworkOptimizer:
    ...
    def _pre_check_data_consistency(self):
        ...
        missing_drugs = []
        found_ses = set()
        missing_se_count = 0  # общее количество записей о не найденных побочных эффектах

        print("graph_node_names", graph_node_names, end="\n\n")

        for drug_name, se_list in self.orlov_data.items():
            # print("drug_name", drug_name, end="\n\n")
            # print("se_list", se_list, end="\n\n")
    ...
    #     # print("sorted_nodes: ", len(sorted_nodes))
    #     # print("network:", len(network))
    #     # print("self.graph.prob_data:", self.graph.prob_data)

    #     probabilities = {}
    #     for node_id in sorted_nodes:
    #         node = network[node_id]
    #         if node_id in evidence:
    #             probabilities[node_id] = evidence[node_id]                                     # ВОЗМОЖНО ПОПРАВИТЬ НА ' "" = 1 '
    #             continue
    #         if not node.parents:
    ...
    #             # print("parent_states_tuple:", parent_states_tuple, end="\n\n")
    #             # print("prob_node_given_parents:", prob_node_given_parents, end="\n\n")
    #             # print("node.parents:", node.parents, end="\n\n")
    ...
    #         probabilities[node_id] = round(expected_prob_sum, 6)
    ...
    def calculate_probabilities_with_evidence(self, evidence=None):
        """
        Вычисляет маргинальные вероятности P(Node=1) для всех узлов в сети,
        учитывая предоставленные "доказательства" (evidence), с использованием
        векторизованных операций NumPy для ускорения вычислений.
        """
        if evidence is None:
            evidence = {}
        ...
        for node_id in sorted_nodes:
            ...
            # Предполагается, что node.parent_configs и node.cond_probs уже являются
            # NumPy-массивами, подготовленными на этапе инициализации узла.
            parent_configs = node.parent_configs      # shape: (N, P)
            cond_probs = node.cond_probs              # shape: (N,)

            # Собираем вероятности родителей в порядке, соответствующем parent_configs
            p_parents = np.array([probabilities[parent_id] for parent_id in node.parents])  # shape: (P,)

            # Транслируем p_parents до формы (N, P)
            # cond_probs и parent_configs уже имеют N строк
            p_expanded = np.broadcast_to(p_parents, parent_configs.shape)  # shape: (N, P)
            ...
            prob_parents_config = np.prod(prob_matrix, axis=1)  # shape: (N,)

            # Взвешенная сумма: Σ P(Node=1 | config) * P(config)
            expected_prob = np.sum(cond_probs * prob_parents_config)

            # Округление для согласованности с оригинальной версией
    ...
    def _initialize_probabilities_and_mapping(self):
        ...
        # print("self.graph.network:", self.graph.network, "\n\n")
        # print("self.initial_prob_data:", initial_prob_data, "\n\n")
        # print("id_to_node_obj:", id_to_node_obj, "\n\n")

        idx = 0
        self.optimizable_prob_map = {}
        self.fixed_prob_map = {}

        print("initial_prob_data:", initial_prob_data)
        ...
        # print("self.optimizable_prob_map.items(): ", self.optimizable_prob_map.items(), end="\n\n")
        # print("initial_prob_data: ", initial_prob_data, end="\n\n")
        print("self.optimizable_prob_map: ", self.optimizable_prob_map, end="\n\n")
        print("self.fixed_prob_map: ", self.fixed_prob_map, end="\n\n")
        print("self.x0: ", self.x0, end="\n\n")
        
        log_to_file(self.logfile_path, f"  Найдено {len(self.optimizable_prob_map)} обучаемых параметров.")
        log_to_file(self.logfile_path, f"  Обучаемые параметры: {self.optimizable_prob_map}.")

        log_to_file(self.logfile_path, f"  Найдено {len(self.fixed_prob_map)} фиксированных параметров из Orlov.json.")
        log_to_file(self.logfile_path, f"  Фиксированные параметры: {self.fixed_prob_map}.")

        log_to_file(self.logfile_path, f"  self.graph.prob_data: {self.graph.prob_data}.")
        log_to_file(self.logfile_path, f"  initial_prob_data: {initial_prob_data}.")
        log_to_file(self.logfile_path, f"  flatten_prob: {self.x0}.")
    ...
    #     current_prob_data = json.loads(json.dumps(self.graph.prob_data))
    #     for (node_name, conf_str), idx in self.optimizable_prob_map.items():
    #         current_prob_data[node_name][conf_str] = round(float(x_flat[idx]), 6)
    #     for (node_name, conf_str), val in self.fixed_prob_map.items():
    #         current_prob_data[node_name][conf_str] = round(float(val), 6)
    #     return current_prob_data


    def _unflatten_probabilities(self, x_flat):
        """Преобразует плоский вектор параметров обратно в полную структуру CPT с округлением."""
        # Глубокое копирование исходной структуры, совместимое с кортежами в ключах
        current_prob_data = copy.deepcopy(self.graph.prob_data)
        
        # Обновление оптимизируемых параметров
    ...
    def _calculate_loss(self, x_flat, log_details=False):
        ...
        # print(" OLD self.graph.prob_data:", self.graph.prob_data, end="\n\n")
        self.graph.update_prob_data_in_network(current_prob_data)
        # print(" NEW self.graph.prob_data:", self.graph.prob_data, end="\n\n")
        ...
        # print(" prepare_nodes:", prepare_nodes, end="\n\n")
        # print(" side_e_nodes:", side_e_nodes, end="\n\n")
        ...
        for drug_name, se_data_list in self.orlov_data.items():
            ...
            # print("calculated_marginals:", calculated_marginals)
            # log_to_file(self.logfile_path, f"calculated_marginals: {calculated_marginals}")
    ...
    def _calculate_gradients(self, x_flat):
        ...
        for i in range(len(x_flat)):
            ...
            x_plus_h[i] += h
            x_minus_h[i] -= h
            # print(f" {i} x_plus_h:", x_plus_h)
            # print(f" {i} x_minus_h:", x_minus_h)
    ...
    def optimize_sgd(self, learning_rate=0.01, epochs=500, log_interval=10, intermediate_file='intermediate_probabilities_opt.json'):
        ...
        # print("self.optimizable_prob_map", self.optimizable_prob_map, end='\n\n')
        print("sorted_params", sorted_params, end='\n\n')

        for (node_name, conf_str), idx in sorted_params:
            log_to_file(self.logfile_path, f"    P({node_name}|{conf_str if conf_str else '""'}) [idx:{idx}] = {current_params[idx]:.6f}", add_timestamp=False)
        log_to_file(self.logfile_path, "  -------------------------------------------------", add_timestamp=False)
...
def save_optimized_probabilities(optimized_prob_data, filename="probabilities_opt.json"):
    ...
    print("optimized_prob_data", optimized_prob_data)

    def convert_to_json_format(data):
        result = {}
        for key, value_dict in data.items():
            converted = {}
            for tuple_key, prob in value_dict.items():
                if tuple_key == ():
                    str_key = ""
                else:
                    # Преобразуем кортеж в строку через запятую без пробелов
                    str_key = ",".join(map(str, tuple_key))
                converted[str_key] = prob
            result[key] = converted
        return result

    converted_prob_data = convert_to_json_format(optimized_prob_data)

    with open(filename, 'w', encoding='utf-8') as f:
        json.dump(converted_prob_data, f, ensure_ascii=False, indent=4)

    # log_to_file("opt_calc.txt", f"Оптимизированные CPT сохранены в: {filename}")

def debag_save_optimized_probabilities(graph, filename="probabilities_opt.json"):
    ...
    for node_id in sorted_nodes:
        ...
        print('node:', node)
        print('node.name:', node.name,)
        print('node.parents_id:', node.parents)
...
if __name__ == "__main__":
    graph_path = 'bayes_network\\data\\graphs\\graphs_4_1.json'
    # graph_path = 'bayes_network\\data\\graphs\\TEST.json'
    orlov_path = 'bayes_network\\data\\Orlov.json'
    logfile_path = 'bayes_network\\data\\opt_calc.txt'
    ...
    network = BayesianGraph(graph_from_json, logfile_path = logfile_path)
    optimazer = BayesianNetworkOptimizer(network, orlov_dataset, logfile_path = logfile_path)

    final_prob_data, final_loss = optimazer.optimize_sgd(epochs = 10, log_interval=1)

    # print("prob_data:", network.prob_data)
    save_optimized_probabilities(final_prob_data, "bayes_network\\data\\probabilities_opt_4_1.json")
    debag_save_optimized_probabilities(network, "bayes_network\\data\\debug_probabilities_opt.json")
